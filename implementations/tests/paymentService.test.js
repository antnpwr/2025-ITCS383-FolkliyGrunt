...
jest.mock('stripe', () => {
  return jest.fn(() => ({
    ...
    customers: { create: mockCustomersCreate, createSource: mockCustomersCreateSource },
...
describe('Payment Service', () => {
  ...
  describe('processPayment', () => {
    test('BANK_TRANSFER returns simulated transaction', async () => {
      const result = await paymentService.processPayment({
        booking_id: 'b1', amount: 200, method: 'BANK_TRANSFER', transfer_reference: 'REF001'
      });
    ...
    test('PROMPTPAY returns simulated transaction', async () => {
      const result = await paymentService.processPayment({
        booking_id: 'b2', amount: 100, method: 'PROMPTPAY'
      });
      expect(result.success).toBe(true);
    ...
    test('CREDIT_CARD with customer_id uses Payment Intent', async () => {
      ...
      const result = await paymentService.processPayment({
        booking_id: 'b3', amount: 500, method: 'CREDIT_CARD', customer_id: 'cus_test'
      });
      expect(result.success).toBe(true);
      expect(result.transaction_id).toBe('pi_test_123');
    });

    test('CREDIT_CARD fallback uses legacy Charges API', async () => {
      const result = await paymentService.processPayment({
        booking_id: 'b4', amount: 300, method: 'CREDIT_CARD', credit_card_token: 'tok_visa'
      });
      expect(result.success).toBe(true);
      expect(result.transaction_id).toBe('ch_test_123');
      expect(mockChargesCreate).toHaveBeenCalled();
    });
    ...
    test('throws on Stripe error', async () => {
      ...
      await expect(paymentService.processPayment({
        booking_id: 'b5', amount: 500, method: 'CREDIT_CARD', credit_card_token: 'tok_bad'
  ...
  describe('processRefund', () => {
    ...
    test('refunds a Payment Intent (pi_xxx)', async () => {
      ...
      expect(result.success).toBe(true);
      expect(mockRefundsCreate).toHaveBeenCalledWith({ payment_intent: 'pi_test_456' });
    });
  ...
  describe('getOrCreateCustomer', () => {
    test('returns existing customer ID from database', async () => {
      pool.query.mockResolvedValue({ rows: [{ stripe_customer_id: 'cus_existing' }] });

      const result = await paymentService.getOrCreateCustomer('auth-1', 'user@test.com');
    ...
    test('creates new customer if none exists', async () => {
      ...
      const result = await paymentService.getOrCreateCustomer('auth-2', 'new@test.com');
  ...
  describe('getSavedCards', () => {
    test('returns mapped card list', async () => {
      mockPaymentMethodsList.mockResolvedValue({
        data: [{
          id: 'pm_1', card: { brand: 'visa', last4: '4242', exp_month: 12, exp_year: 2026 }
        }]
      });

      const cards = await paymentService.getSavedCards('cus_test');
      expect(cards).toEqual([{ id: 'pm_1', brand: 'visa', last4: '4242', exp_month: 12, exp_year: 2026 }]);
    });
  });
