...
describe('Auth API Endpoints', () => {
  ...
  describe('POST /api/auth/register', () => {
    ...
    it('should default role to CUSTOMER when no role specified', async () => {
      ...
      const response = await request(app)
        .post('/api/auth/register')
        .send({ email: 'cust@test.com', password: 'Pass123!', full_name: 'Cust' });

      expect(response.status).toBe(201);
      expect(Profile.create).toHaveBeenCalledWith(expect.objectContaining({ role: 'CUSTOMER' }));
    });
    ...
    it('should allow ADMIN role when explicitly specified', async () => {
      ...
      const response = await request(app)
        .post('/api/auth/register')
        .send({ email: 'admin@test.com', password: 'Pass123!', full_name: 'Admin', role: 'ADMIN' });

      expect(response.status).toBe(201);
      expect(Profile.create).toHaveBeenCalledWith(expect.objectContaining({ role: 'ADMIN' }));
    });
    ...
    it('should return 500 if Profile.create throws', async () => {
      ...
      const response = await request(app)
        .post('/api/auth/register')
        .send({ email: 'err@test.com', password: 'Pass123!', full_name: 'Err' });
